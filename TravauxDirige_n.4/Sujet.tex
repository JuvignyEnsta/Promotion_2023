...
devra partionner sa boucle. La table suivante résume les 
caractéristiques des différents partitioneurs (le grain correspond au 
nombre d'itération  que doit faire une boucle après avoir effectuer 
...
uniforme est créé sauf si la taille du grain empêche de créer $P$ parties &
...
\begin{enumerate}
    \item Faites en sorte que désormais, une tâche correspond au calcul de $nt$ lignes
(et non d'une seule ligne).
    \item Paralléliser à l'aide d'OpenMP le calcul dans une tâche des $nt$ lignes à l'aide de 
    $nt$ threads (on rajoutera la clause \texttt{num_threads}(nt) dans le \texttt{\#pragma omp})
    \item Regardez les différentes accélérations obtenues en fonction du nombre de processus MPI et 
    du paramètre $nt$ et regarder quel est le meilleur temps que vous obtenez.
...
Paralléliser à l'aide de TBB le code de raytracing fourni avec le TP.
Essayer divers partitioneurs et tailles de grain et montrez à l'aide des 
